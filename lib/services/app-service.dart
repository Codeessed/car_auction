...
class AppService {
  ...
  Future<CarDetailsModel?> getCarDetails(String id) async {
    try {
      var response = await client.get(Uri.parse("${BASEURL}inventory/car/$id"), headers: {
        "Content-type": "application/json",
      });
      var data = jsonDecode(response.body);
  ...
  Future<CarMediaResponseModel> getCarMedia(String id) async {
    try {
      var response = await client.get(Uri.parse("${BASEURL}inventory/car_media").replace(
        queryParameters: {
          'carId' : id
        },
      ), headers: {
      ...
      CarMediaResponseModel serverResponse = CarMediaResponseModel.fromJson(data) ;
